...
3. High Traffic:
   ...
   To solve this, we are implementing a ReceiptParsingWorker to ensure that the ReceiptParsingService does not get overloaded. The Worker will pop events from the Redis queue at certain time intervals and then call the ReceiptParsingService to perform the parsing logic. The Service contains the core logic of parsing a receipt while the Worker acts as a middleman to call the Service whenever necessary. This prevents direct access to the Service and allows for a middleman to control the amount of requests. The Service will only need to process the events that the Worker is sending. Additionally, with high traffic, the Redis queue can get "full" but this only happens if you run out of physical memory. In cases like this, Redis would return an error to the server saying it is out of memory.
